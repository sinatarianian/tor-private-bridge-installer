...
sanitize_digits() {
  # remove ALL whitespace (spaces, tabs, CR, LF) and keep only digits
  # (If user typed junk, validation will fail.)
  local v="$1"
...
resolve_port() {
  ...
  desired="$(sanitize_digits "$desired")"
...
collect_config() {
  ...
  # Final hard sanitation (prevents bare-number line bugs)
  OR_PORT="$(sanitize_digits "$OR_PORT")"
  PT_PORT="$(sanitize_digits "$PT_PORT")"
  validate_numeric_port "OR_PORT" "$OR_PORT"
  validate_numeric_port "PT_PORT" "$PT_PORT"
}
...
write_torrc() {
  ...
  # Use printf-safe vars (sanitized already)
  local orp="$OR_PORT"
  local ptp="$PT_PORT"
  local email="$EMAIL"
...
ServerTransportListenAddr obfs4 0.0.0.0:${ptp}
